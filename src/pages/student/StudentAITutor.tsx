...
export const StudentAITutor = () => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: "You are a helpful, encouraging, and knowledgeable AI Tutor for students at STAHIZA HUB. Your goal is to explain complex concepts in simple terms, provide step-by-step solutions to problems, and encourage critical thinking. Use markdown for formatting, including bold text, lists, and code blocks where appropriate. If a student asks a question outside of education or school management, gently redirect them back to their studies.",
        },
      });
      ...
      const errorMsg: Message = {
        id: Date.now() + 1,
        role: 'ai',
        text: "I'm having some trouble connecting to my brain right now. Please check your connection and try again.",
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
  ...
  const handleSuggestionClick = (s: string) => {
    setMessage(s);
    // Use a small timeout to ensure state is updated before sending
    setTimeout(() => {
                  ...
                  <div className={`flex max-w-[80%] gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      <div className={cn(
                        ...
                        {msg.role === 'ai' ? (
                          <div className="prose prose-sm dark:prose-invert max-w-none prose-p:leading-relaxed prose-pre:bg-navy/5 prose-pre:border prose-pre:border-border">
